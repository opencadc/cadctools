...
def main():
    ...
    put_parser.add_argument('-c', '--compress', help='gzip compress the data',
                            action='store_true', required=False)
    put_parser.add_argument('-r', '--replace', action='store_true', help='replace existing file', required=False)
        ...
        if args.cmd == 'get':
            ...
            if args.fileIDs is not None:
                ...
                if len(file_ids) != len(files):
                    handle_error('Different size of file ID list ({}) and list of files to put ({})'.
                                  format(file_ids, files))
            else:
            ...
            if args.replace:
                # check that the file IDs exist
                non_exist = []
                for file_id in file_ids:
                    if client.get_file_info(archive, file_id) is None:
                        non_exist.append(file_id)
                if len(non_exist) == 1:
                    handle_error('File ID {} to replace not found'.format(non_exist[0]))
                elif len(non_exist) > 0:
                    handle_error('Following file IDs to replace not found'.format(non_exist))
