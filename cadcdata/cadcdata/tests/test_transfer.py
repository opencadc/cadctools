...
def test_transfer_constructor():
    ...
    with pytest.raises(TransferError):
        Transfer(test_target_good, test_dir_put,
                 protocols=[Protocol(i) for i in
                            DIRECTION_PROTOCOL_MAP['pullFromVoSpace']])
    ...
    with pytest.raises(TransferError):
        Transfer(test_target_good, test_dir_put,
                 protocols=[Protocol(i) for i in
                            DIRECTION_PROTOCOL_MAP['pushToVoSpace']],
    ...
    assert '1234' == tran.properties['LENGTH']
...
def test_roundtrip_put():
    ...
    assert tran.target == tran2.target, 'Wrong target.'
    assert tran.direction == tran2.direction, 'Wrong direction.'
    assert tran.properties == tran2.properties, 'Wrong properties.'
    assert len(tran.protocols) == len(tran2.protocols),\
        'Wrong number of protocols.'
    for i in range(len(tran.protocols)):
        p1 = tran.protocols[i]

        assert p1.uri == p1.uri, 'Wrong uri, protocol %i' % i
        assert p1.endpoint == p1.endpoint, 'Wrong endpoint, protocol %i' % i


def test_roundtrip_get():
    tran = Transfer(test_target_good, test_dir_get,
                    protocols=[Protocol(i, endpoint='http://somewhere') for
                               i in DIRECTION_PROTOCOL_MAP['pullFromVoSpace']],
                    properties={'LENGTH': '1234',
